...
int main(){
	...
	/*
	 * p.crc = (uint8_t)packetData & (0x3<<0);
	 * p.status = (uint8_t)packetData & (0x1<< 3);
	 * p.payload = (uint16_t)packetData & (0xFFF<<4);
	 * p.bat = (uint8_t)packetData & (0x7<<15);
	 * p.sensor = (uint8_t)packetData & (0x7<<18);
	 * p.longAddr = (uint8_t)packetData & (0xFF<<21);
	 * p.shortAddr = (uint8_t)packetData & (0x3<<29);
	 * p.addrMode = (uint8_t)packetData & (0x1<<31);
	 * */
